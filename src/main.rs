...
#[derive(Parser)]
#[command(name = "confctl", version, about = "A simplified jq for configuration files (JSON, YAML, TOML)")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
fn detect_format(file_path: &str) -> Result<Format> {
    ...
    match ext.as_deref() {
        ...
        Some(other) => bail!("Unsupported file extension: .{other}. Supported: .json, .yaml, .yml, .toml"),
    ...
    let value = match format {
        ...
        Format::Toml => {
            ...
            let json_str = serde_json::to_string(&toml_value)
                .context("Failed to serialize TOML to JSON")?;
            serde_json::from_str::<Value>(&json_str)
                .context("Failed to deserialize TOML-JSON")?
        }
    };
...
mod tests {
    ...
    fn test_resolve_simple_key() {
        let data = json!({"club": {"name": "Arsenal", "founded": 1886}});
        let result = resolve_path(&data, "club.name").unwrap();
        assert_eq!(result, &json!("Arsenal"));
    }

    #[test]
    fn test_resolve_numeric_index() {
        let data = json!({"players": [{"name": "Vinicius"}, {"name": "Bellingham"}]});
        let result = resolve_path(&data, "players.1.name").unwrap();
        assert_eq!(result, &json!("Bellingham"));
    }

    #[test]
    fn test_resolve_missing_key() {
        let data = json!({"club": {"name": "Barcelona"}});
    ...
    fn test_resolve_index_out_of_bounds() {
        let data = json!({"trophies": [1, 2, 3]});
        let result = resolve_path(&data, "trophies.5");
        assert!(result.is_err());
    ...
    fn test_resolve_scalar_traversal() {
        let data = json!({"name": "Real Madrid"});
    ...
    fn test_format_string_no_quotes() {
        assert_eq!(format_value(&json!("Manchester City")), "Manchester City");
    }
